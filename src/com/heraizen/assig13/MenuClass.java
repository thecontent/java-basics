...
class profile_details {
	...
	public void start(profile_list[] arr) {
		...
		do {
			...
			System.out.println("1.ADD\t2.VIEW\t3.EDIT\t4.DELETE\t5.SEARCH\t6.SORT\t7.EXIT ");
			System.out.println(
					"\n------------------------------------------------------------------------------------------\n");
	...
	void emailSort(profile_list[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			System.out.format("\n%-7s%-15s%-15s%-25s%3d", arr[i].Id, arr[i].Name, arr[i].Mobile_number, arr[i].Email,
					arr[i].age);
		}
	}
	...
	void editEach() {
		...
		System.out
				.println(list.Id + "  " + list.Name + "  " + list.Mobile_number + "  " + list.Email + "  " + list.age);
	...
	void Add_employee() {
		System.out.println("Size before - "+details_array.length);
		int var;
		System.out.println(details_array.length % 5);
		System.out.println(details_array.length % 10);
		if ((details_array.length % 5 == 1 || details_array.length % 5 == 2) || (details_array.length % 10 == 1 || details_array.length % 10 == 2)) {	
			var = details_array.length + 5;
		}
		else {
			var = details_array.length + 1;
		}
		profile_list[] Dlist = new profile_list[var];
		System.out.println("Size before - "+Dlist.length);
	...
	void ViewAll() {
		for (int i = 0; i < details_array.length; i++) {
			System.out.format("\nID : %-7sName : %-15s   Mobile Number : %-15s Email : %-25s  Age : %d",
					details_array[i].Id, details_array[i].Name, details_array[i].Mobile_number, details_array[i].Email,
					details_array[i].age);
		}
	}
